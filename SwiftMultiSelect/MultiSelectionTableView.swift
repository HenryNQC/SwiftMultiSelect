...
extension MultiSelecetionViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        activityIndicator.start {
            SwiftMultiSelect.delegate?.loadMoreItemTrigger()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Set initial state
        if let itm_pre = selectedItems.firstIndex(where: { itm -> Bool in
            itm == item
        }) {
            selectedItems[itm_pre].color = cell.initials.backgroundColor!
    ...
    func reloadCellState(row: Int, selected: Bool) {
        if let cell = tableView.cellForRow(at: IndexPath(row: row, section: 0)) as? CustomTableCell {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if cell.accessoryType == UITableViewCell.AccessoryType.checkmark {
            ...
            reloadAndPositionScroll(itemId: item.id, remove: true, row: indexPath.row)
        } else {
            // Set accessory type
            cell.accessoryType = UITableViewCell.AccessoryType.checkmark
            ...
            // Reload collectionview
            reloadAndPositionScroll(itemId: item.id, remove: false, row: indexPath.row)
        }
...
//        }
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        if searchText.isEmpty {
            perform(#selector(hideKeyboardWithSearchBar(_:)), with: searchBar, afterDelay: 0)
            searchString = ""
        }

        SwiftMultiSelect.delegate?.userDidSearch(searchString: searchText)
        // Remove because tableView just need to reload when has new data
//        self.tableView.reloadData()
    }
