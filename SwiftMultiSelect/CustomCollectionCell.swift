...
class CustomCollectionCell: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        labelTitle.frame = CGRect(
            x: Double(imageAvatar.frame.origin.x),
            y: Double(imageAvatar.frame.origin.y + imageAvatar.frame.size.height),
            width: Double(bounds.width - (2 * imageAvatar.frame.origin.x)),
            height: 20
        )
