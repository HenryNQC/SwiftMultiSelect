...
//            self.navigationItem.rightBarButtonItem?.isEnabled = (self.selectedItems.count > 0)
    ...
    var navBarHeight: CGFloat {
        if navigationController != nil {
            return navigationController!.navigationBar.frame.size.height + (UIApplication.shared.isStatusBarHidden ? 0 : 20)
        } else {
            return 0
        }
    }
    ...
    func createViewsAndSetConstraints() {
        ...
        separator.frame = CGRect(x: 0.0, y: Config.selectorStyle.selectionHeight - Config.selectorStyle.separatorHeight, width: Double(view.frame.size.width), height: Config.selectorStyle.separatorHeight)
    ...
    @objc public func selectionDidEnd() {
        ...
        SwiftMultiSelect.delegate?.swiftMultiSelect(didSelectItems: selectedItems)
    }
    ...
    override func viewDidLoad() {
        ...
        navigationItem.leftBarButtonItem = leftButtonBar
        ...
        view.backgroundColor = Config.mainBackground
